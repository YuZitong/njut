...
\begin{abstract}
\kaishu\zihao{-4}
X射线CT~（Computed Tomography，即计算机断层扫描）重建，是一种医学影像技术。
它通过X射线所产生数据来重建被投射物体的断层影像，
由于断层影像给诊断、治疗所带来的便利，CT技术被广泛应用于医疗领域。

对于传统采用一维探测器的二维切片CT重建，使用二维探测器
直接三维重建可以以更快的速度
获取数据，同时也能更好保留三维物体的局部细节。此外，有限角度CT重建相对于全角度CT重建，
采样速率更高，因此减小时间过长对数据造成的干扰，同时也避免过多照射对病人的损害。但是
有限角度重建在没有先验知识的情况下，一般是无法稳定重建被探测物体，会产生边缘模糊等问题。
在本文中，将研究如何利用先验知识，来提高在有限角度下直接三维CT成像的质量。

考虑到
SART算法~（Simultaneous Algebraic Reconstruction Technique）在直接三维CT成像上，具有适应不同几何关系的系统、
可以嵌入先验知识、能够对有限角度CT投影数据进行重建的特点，本文
在SART的基础上提出一种优化算法框架。该框架将SART算法写成优化目标的形式，并将先验知识的约束
添加到优化目标里，从而可以较好融合先验知识与投影数据进行重建。

本论文将针对特定的三维物体，
从超声成像~（UltraSound Imaging）重建的数据中提取出梯度信息作为先验知识，代入到优化算法框架里并展开为具体特定的算法，
由此来重建CT成像。得到的优化图像与直接CT成像相比，在物体内部结构细节探测上，尤其是边缘探测上表现很好，一定程度
上补偿了有限角度的劣势。

\keywords{CT成像；有限角度；先验知识；优化；SART算法；超声成像}
\end{abstract}
...
\englishkeywords{CT; limited angle; prior knowledge; optimization problem; SART; Ultrasound Imaging}
\end{englishabstract}
\NJUTtableofcontents
\mainmatter
...
现在，矩阵$\B$和$\C$ 都已经得到了，我们需要对式~\eqref{eq:gradout2}作进一步展开，并得到可以计算的最终结果。
